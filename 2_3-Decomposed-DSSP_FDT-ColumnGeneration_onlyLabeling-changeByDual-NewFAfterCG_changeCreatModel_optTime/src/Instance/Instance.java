...
//        this.tripWithDriveInfos =new TripWithDriveInfos[nbTrips];
    ...
    public int getNbNodes() {
        return 2 + 2 * nbDepot + nbTrips;
    }

    public double getMaxPercentageOfCombinedTrip() {
        return this.maxPercentageOfCombinedTrip;
    }
    ...
    public void setDepot(int i, Depot depot) {
        this.depots[i] = depot;
    }
...
//    }
    ...
    public int getShortConnectionTimeForVehicle() {
        return shortConnectionTimeForVehicle;
    }
...
//    }
    ...
    public int getIdleTimeCostForDriverPerUnit() {
        return this.idleTimeCostForDriverPerUnit;
    }

    public int getIdleTimeCostForVehiclePerUnit() {
        return this.idleTimeCostForVehiclePerUnit;
    }

    public int getTimeSlotUnit() {
        return this.timeSlotUnit;
    }

    public int getStartingPlanningHorizon() {
        return this.startingPlanningHorizon;
    }

    public int getEndingPlaningHorizon() {
        return this.endingPlaningHorizon;
    }
...
//        return whetherHavePossibleArc;
    ...
    public boolean whetherConArcEffectByTW(Trip trip) {// re-think whether have possible arc effect
        boolean whetherEffect = false;
        boolean whetherEffectOut = false;
        boolean whetherEffectIn = false;
        for (int t = 0; t < this.getNbTrips(); t++) {
            Trip tripCon = this.getTrip(t);
            if (tripCon.getIdOfTrip() != trip.getIdOfTrip()) {
                boolean whetherPossibleArcOut = this.whetherHavePossibleArcAfterCleaning(trip.getIdOfTrip(), tripCon.getIdOfTrip());
                boolean whetherPossibleArcIn = this.whetherHavePossibleArcAfterCleaning(tripCon.getIdOfTrip(), trip.getIdOfTrip());
                if (whetherPossibleArcOut) {
                    int realMinDifferenceTimeOut = tripCon.getEarliestDepartureTime() - (trip.getLatestDepartureTime() + trip.getDuration());

                    if (realMinDifferenceTimeOut < this.getMinPlanTurnTime()) {
                        whetherEffectOut = true;

                    }
                }

                if (whetherPossibleArcIn) {
                    int realMinDifferenceTimeIn = trip.getEarliestDepartureTime() - (tripCon.getLatestDepartureTime() + tripCon.getDuration());

                    if (realMinDifferenceTimeIn < this.getMinPlanTurnTime()) {
                        whetherEffectIn = true;
                    }

                }
            }
        }
        if (whetherEffectIn || whetherEffectOut) {
            whetherEffect = true;
        }
    ...
    public int getNbTripCouldStartFromDepot(){
        int nbTripCouldStartFromDepot=0;
        for(int k=0;k<nbDepot;k++){
            Depot depot= this.getDepot(k);
            int idCityAsDepot=depot.getIdOfCityAsDepot();
            for(int t=0;t<nbTrips;t++){
                Trip trip= this.getTrip(t);
                int idStartCityOfTrip=trip.getIdOfStartCity();
                if(idStartCityOfTrip==idCityAsDepot){
                    nbTripCouldStartFromDepot++;
                }

            }
        }
        return nbTripCouldStartFromDepot;
    }

    public boolean whetherCouldHavePreTripInGraph(Trip trip) {
        boolean whetherCouldHavePreTripInGraph = true;
        int nbConnectionTrip = 0;
        for (int i = 0; i < this.getNbTrips(); i++) {
            Trip tripBefore = this.getTrip(i);
            if (tripBefore.getIdOfTrip() != trip.getIdOfTrip()) {
                if (whetherHavePossibleArcAfterCleaning(tripBefore.getIdOfTrip(), trip.getIdOfTrip())) {
                    whetherCouldHavePreTripInGraph = true;
                    nbConnectionTrip++;
                }
            }
        }
        if (nbConnectionTrip < 1) {
            whetherCouldHavePreTripInGraph = false;
        }
        return whetherCouldHavePreTripInGraph;
    }

    public boolean whetherCouldHaveSuccessorTripInGraph(Trip trip) {
        boolean whetherCouldHaveSucessorTripInGraph = true;
        int nbConnectionTrip = 0;
        for (int i = 0; i < this.getNbTrips(); i++) {
            Trip tripSuccessor = this.getTrip(i);
            if (tripSuccessor.getIdOfTrip() != trip.getIdOfTrip()) {
                if (whetherHavePossibleArcAfterCleaning(trip.getIdOfTrip(), tripSuccessor.getIdOfTrip())) {
                    whetherCouldHaveSucessorTripInGraph = true;
                    nbConnectionTrip++;
                }
            }
        ...
        if (nbConnectionTrip < 1) {
            whetherCouldHaveSucessorTripInGraph = false;
        }
        return whetherCouldHaveSucessorTripInGraph;
    }

    public int getKeyDeparturePointForExtendFrom(Trip trip) {
        int keyDepOfCurrent = trip.getLatestDepartureTime();
        for (int i = 0; i < this.getNbTrips(); i++) {
            Trip latterTrip = this.getTrip(i);
            if (this.whetherHavePossibleArcAfterCleaning(trip.getIdOfTrip(), latterTrip.getIdOfTrip())) {
                int realConnection = latterTrip.getEarliestDepartureTime() - trip.getLatestDepartureTime() - trip.getDuration();
                if (realConnection < minPlanTurnTime) {
                    int earliestDepF = trip.getEarliestDepartureTime();
                    int latestDepF = trip.getLatestDepartureTime();
                    int earliestDepS = latterTrip.getEarliestDepartureTime();
                    int latestDepS = latterTrip.getLatestDepartureTime();
                    int key = earliestDepS - minPlanTurnTime - trip.getDuration();//改成2025.4.1
                    if (key > latestDepF) {
                        key = latestDepF;
                    }
                    if (key < earliestDepF) {
                    ...
                    if (key >= earliestDepF && key <= latestDepF && key < keyDepOfCurrent) {
                        keyDepOfCurrent = key;
                    }
                }
            }
        }
        return keyDepOfCurrent;
    }
    ...
    public ArrayList<Trip> getPreTrips(Trip currentTrip) {
        int idOfCurrentStartCity = currentTrip.getIdOfStartCity();
        int latestCurrentDepartureTime = currentTrip.getLatestDepartureTime();
        ArrayList<Trip> tripPreList = new ArrayList<>();
        for (int i = 0; i < nbTrips; i++) {
            Trip trip = this.getTrip(i);
            int idOfTripEndCity = trip.getIdOfEndCity();
            int earliestTripDepartureTime = trip.getEarliestDepartureTime();
            int durationTrip = trip.getDuration();
            int earliestTripArrivalTime = earliestTripDepartureTime + durationTrip;
            if (idOfTripEndCity == idOfCurrentStartCity) {
                if (latestCurrentDepartureTime - earliestTripArrivalTime >= this.minPlanTurnTime) {
                    tripPreList.add(trip);
                }
            }
        }
        return tripPreList;
    ...
    public ArrayList<Trip> getSuccessTrips(Trip currentTrip) {
        int idOfCurrentEndCity = currentTrip.getIdOfEndCity();
        int earliestCurrentDepartureTime = currentTrip.getEarliestDepartureTime();
        int durationCurrent = currentTrip.getDuration();
        int earliestCurrentArrivalTime = earliestCurrentDepartureTime + durationCurrent;
        ArrayList<Trip> tripSuccessList = new ArrayList<>();
        for (int i = 0; i < nbTrips; i++) {
            Trip trip = this.getTrip(i);
            int idOfTripStartCity = trip.getIdOfStartCity();
            int latestTripDepartureTime = trip.getLatestDepartureTime();
            if (idOfCurrentEndCity == idOfTripStartCity) {
                if (latestTripDepartureTime - earliestCurrentArrivalTime >= this.minPlanTurnTime) {
                    tripSuccessList.add(trip);
                }
            }
        }
        return tripSuccessList;

    }

    public boolean whetherTripEffectByTWOut(Trip trip) {
        boolean whetherEffectByLatterTrip = false;
        int earliestDep = trip.getEarliestDepartureTime();
        int latestDep = trip.getLatestDepartureTime();
        int idStartCity = trip.getIdOfStartCity();
        int idEndCity = trip.getIdOfEndCity();
        int durationTrip = trip.getDuration();
        for (int j = 0; j < nbTrips; j++) {
            Trip trip1 = this.getTrip(j);
            int idStartCity1 = trip1.getIdOfStartCity();
            int idEndCity1 = trip1.getIdOfEndCity();
            int earliestDep1 = trip1.getEarliestDepartureTime();
            int latestDep1 = trip1.getLatestDepartureTime();
            int duration1 = trip1.getDuration();

            if (whetherHavePossibleArcAfterCleaning(trip.getIdOfTrip(), trip1.getIdOfTrip())) {

                if (idStartCity1 == idEndCity) {// come out from this city
                    int realMinConTime = earliestDep1 - (latestDep + durationTrip);
                    if (realMinConTime < minPlanTurnTime) {
                        whetherEffectByLatterTrip = true;
        ...
        return whetherEffectByLatterTrip;
    }


    public boolean whetherTripEffectByTWIn(Trip trip) {
        boolean whetherEffectByFormerTrip = false;
        int earliestDep = trip.getEarliestDepartureTime();
        int latestDep = trip.getLatestDepartureTime();
        int idStartCity = trip.getIdOfStartCity();
        int idEndCity = trip.getIdOfEndCity();
        ...
        for (int j = 0; j < nbTrips; j++) {
            ...
            int latestDep1 = trip1.getLatestDepartureTime();
            int duration1 = trip1.getDuration();
            if (whetherHavePossibleArcAfterCleaning(trip1.idOfTrip, trip.getIdOfTrip())) {
                if (idEndCity1 == idStartCity) {// come into this city
                    int realMinConTime = earliestDep - (latestDep1 + duration1);
                    System.out.println("real time into " + realMinConTime);
                    if (realMinConTime < minPlanTurnTime) {
                        whetherEffectByFormerTrip = true;
                    }
                }
            }
        }
        return whetherEffectByFormerTrip;
    }


    public boolean whetherTripEffectByTWBothSide(Trip trip) {
        boolean whetherEffectByBothSide = false;
        int earliestDep = trip.getEarliestDepartureTime();
        int latestDep = trip.getLatestDepartureTime();
        int idStartCity = trip.getIdOfStartCity();
        int idEndCity = trip.getIdOfEndCity();
        ...
        if (whetherEffectByFormerTrip == true && whetherEffectByLatterTrip == true) {
            whetherEffectByBothSide = true;
        }

        return whetherEffectByBothSide;
    }


    public ArrayList<TripWithDepartureTime> getKeyTripWithDepTimes(){
        ArrayList<TripWithDepartureTime> tripArrayList=new ArrayList<>();
        for(int i=0;i<nbTrips;i++){
            Trip trip=this.getTrip(i);
            int keyTime=Integer.MAX_VALUE;
            int keyTimeByFollow=trip.latestDepartureTime;
            int keyTimeByPre=trip.earliestDepartureTime;
            if(this.whetherTripEffectByTWBothSide(trip)){
                int earliestDep = trip.getEarliestDepartureTime();
                int latestDep = trip.getLatestDepartureTime();
                int idStartCity = trip.getIdOfStartCity();
                int idEndCity = trip.getIdOfEndCity();
                ...
                for(int j=0;j<nbTrips;j++){
                    Trip tripOther=this.getTrip(j);
                    int earliestDepOther = tripOther.getEarliestDepartureTime();
                    int latestDepOther = tripOther.getLatestDepartureTime();
                    int durationTripOther = tripOther.getDuration();
                    if(this.whetherHavePossibleArcAfterCleaning(trip.idOfTrip,tripOther.getIdOfTrip())) {
                        int realMinConnection = earliestDepOther - (latestDep + durationTrip);
                        if (realMinConnection < this.minPlanTurnTime) {
                            int keyFeasibleDepForConnectingFollowing = latestDepOther - minPlanTurnTime - durationTrip;
                            // Then as long as you choose the time before this value, it would be have at least one feasible solution
                            if(keyFeasibleDepForConnectingFollowing<keyTimeByFollow&&keyFeasibleDepForConnectingFollowing>=earliestDep){
                                keyTimeByFollow=keyFeasibleDepForConnectingFollowing;
                            }
                        }
                    }
                    if(this.whetherHavePossibleArcAfterCleaning(tripOther.getIdOfTrip(),trip.getIdOfTrip())){
                        int realMinConnection=earliestDep-(latestDepOther+durationTripOther);
                        if(realMinConnection<this.minPlanTurnTime){
                            int keyFeasibleDepForConnectionPre=earliestDepOther+durationTripOther+minPlanTurnTime;
                            //Then as long as it choose the time after this ,then it could ensure the preTrip si feasible to connect
                            if(keyFeasibleDepForConnectionPre>keyTimeByPre&&keyFeasibleDepForConnectionPre<=latestDep){
                                keyTimeByPre=keyFeasibleDepForConnectionPre;
                            }
                        }
                    }

                }
                if(keyTimeByPre<=keyTimeByFollow){
                    keyTime=keyTimeByPre;
                }else {
                    System.out.println("seems not any feasible solution");
                }
                TripWithDepartureTime tripWithDepartureTime= new TripWithDepartureTime(trip,keyTime);
                tripArrayList.add(tripWithDepartureTime);
            }
        }
...
//                + "\t" + "minWorkingTime: " + minWorkingTime + "\t"+ "maxWorkingTime: " + maxWorkingTime;
    ...
    public static void main(String[] args) {
        // InstanceReader reader = new InstanceReader("inst_nbCity03_Size90_Day1_nbTrips06_combPer0.0_TW1.txt");
        // InstanceReader reader1=new InstanceReader("inst_ance basic information.txt");
        //to check the key Trip: instance_nbCity03_Size90_Day1_nbTrips05
...
//            System.out.println("Whether Time Window would effect by time window trip_"+i+" "+instance.whetherConArcEffectByTW(instance.getTrip(i)));
//            if(instance.whetherCouldHavePreTripInGraph(instance.getTrip(i))==false) {
//                System.out.println("Whether trip would have a preNode in graph: trip_" + i + " " + instance.whetherCouldHavePreTripInGraph(instance.getTrip(i)));
//            }
//            if(instance.whetherCouldHaveSuccessorTripInGraph(instance.getTrip(i))==false){
//                System.out.println("Whether trip would have a successorNode in graph: trip_"+i+" "+instance.whetherCouldHaveSuccessorTripInGraph(instance.getTrip(i)));
//            }
//
//        }
//        System.out.println("Whether Time Window would effect by time window trip_"+4+" "+instance.whetherConArcEffectByTW(instance.getTrip(4)));
//        System.out.println("duration_4 :"+instance.getTrip(4).getDuration());
//        System.out.println("latestDeparture "+instance.getTrip(4).getLatestDepartureTime());
//        System.out.println("earliestDeparture "+instance.getTrip(5).getEarliestDepartureTime());
//        System.out.println("check the node_3 whether is effect by TW "+instance.whetherConArcEffectByTW(instance.getTrip(3)));
        //
//        System.out.println("keyPoint for trip3 is :"+instance.getKeyDeparturePointForExtendFrom(instance.getTrip(4)));
//        System.out.println("KeyPoint for trip4 is:" +instance.getKeyDeparturePointForExtendFrom(instance.getTrip(5)));
//        System.out.println(instance.getPreTrips(instance.getTrip(2)));
//        System.out.println(instance.getSuccessTrips(instance.getTrip(5)));
        //
        System.out.println("keyPoint for trip4 is :" + instance3.getKeyDeparturePointForExtendFrom(instance3.getTrip(4)));
        System.out.println("KeyPoint for trip2 is:" + instance3.getKeyDeparturePointForExtendFrom(instance3.getTrip(2)));
        System.out.println(instance3.getPreTrips(instance3.getTrip(4)));
        System.out.println(instance3.getSuccessTrips(instance3.getTrip(0)));
        System.out.println(instance3.whetherConArcEffectByTW(instance3.getTrip(2)));
        System.out.println(instance3.whetherTripEffectByTWBothSide(instance3.getTrip(2)));
        System.out.println(instance3.whetherTripEffectByTWIn(instance3.getTrip(2)));
        System.out.println(instance3.whetherTripEffectByTWOut(instance3.getTrip(2)));
        System.out.println("KEY"+ instance3.getKeyTripWithDepTimes());
//        System.out.println(instance3.getTrip(2));
//        System.out.println(instance3.getTrip(3));
