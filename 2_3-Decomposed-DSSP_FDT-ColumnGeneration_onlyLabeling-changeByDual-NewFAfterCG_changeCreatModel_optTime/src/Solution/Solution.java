...
public class Solution {
    ...
    //***************Here I try to add some code for branch and price************************************

    public int getTotalCost() {
        int totalCost = 0;
        for (int k = 0; k < this.driverSchedules.size(); k++) {
            totalCost += getCostC(k);
        }
        return totalCost;
    }

    public void printSummary() {
        System.out.println("NbSchedules: " + driverSchedules.size());
        System.out.println("Total Cost: " + getTotalCost());
        System.out.println("Feasible: " + whetherSolutionIsFeasible());
    }


    // ✅ 新增：用于保存每个 schedule 的变量值（仅在 LP 解中有用）
    private Map<DriverSchedule, Double> scheduleValueMap = new HashMap<>();

    public void setScheduleValue(DriverSchedule schedule, double value) {
        scheduleValueMap.put(schedule, value);
    }

    public Double getScheduleValue(DriverSchedule schedule) {
        return scheduleValueMap.get(schedule);
    }

    public boolean contains(DriverSchedule schedule) {
        return scheduleValueMap.containsKey(schedule);
    }

    //*****************above is prepare for branch and price************************
