...
public class SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost {
...
//    private IloNumVar[][] varWhetherWaitGreaterThanMinPlan;// b_ij^DS1
//    private IloNumVar[][] varWhetherWaitLessThanDriverShort;// b_ij^DS2
    ...
    private IloNumVar[][] varDriving; //o_i^d

    private IloNumVar[][][] varChangeOver; //here is the variable y d i j ()
...
//
    ...
    public SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost(Instance instance, Schedules schedules) throws IloException {
    ...
    public SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost(Instance instance, Schedules schedules, Boolean whetherSolveRelaxationProblem) throws IloException {
    ...
    public Solution solveWithCplexBasedOnGivenSchedules() {
        try {
            ...
            cnstFixedTimeAccoringDriverSchedule();
            ...
            cnstOnePersonDriving();// add 2025.0129
            cnstDrivingTime();// add 2025.0129
            cnstWorkStatusFirst();// add 2025.0129
            cnstWorkStatusSecond();// add 2025.0129
            cnstChangeVehicleBetweenNormalTrips();// add 2025.0129
...
//
//            cnstOneVehicleWaitingTimeForAllCombinedArc_M1();//(36)-1// change 2025.1.22
//            cnstOneVehicleWaitingTimeForAllCombinedArc_M2();//(36)-2// change 2025.1.22
//
//            cnstTwoVehicleWaitingTimeForAllCombinedArc_M1();//new(36)-1// change 2025.1.22
//            cnstTwoVehicleWaitingTimeForAllCombinedArc_M2();//new(36)-1// change 2025.1.22
//
//            cnstVehicleWaitingTimeForNotAllCombinedArc_1();//(35)-1//
//            cnstVehicleWaitingTimeForNotAllCombinedArc_2();//(35)-2//
////
//            cnstOneVehicleWaitingTimeForAllCombinedArc_1();//(37)-1 // change 2025.1.22
//            cnstOneVehicleWaitingTimeForAllCombinedArc_2();//(37)-2// change 2025.1.22
////
//            cnstTwoVehicleWaitingTimeForAllCombinedArc_1();// new(37)-1// change 2025.1.22
//            cnstTwoVehicleWaitingTimeForAllCombinedArc_2();// new(37)-2// change 2025.1.22
//
//            cnstDetectOneVehicleShortConnectionArcForAllCombined();//(40)//change 2025.1.20// change 2025.2.6
//            cnstDetectTwoVehicleShortConnectionArcForAllCombined();//(40)//change 2025.1.20// change 2025.1.22
//
...
//            cplex.setParam(IloCplex.Param.MIP.Tolerances.MIPGap, 1e-09);
...
//                addWarmStartBeforeSolve(warmStartFilePath);
            ...
            if (cplex.solve()) {
                ...
                if (this.whetherSolveRelaxationProblem == false) {
                    ...
                    return getSolutionObtainedByFlowMatrixInformation();
                } else {
                    return null;
                }
            } else {
    ...
    private void analyzeTrip18Problem() {
        System.out.println("=== TRIP 18 DETAILED ANALYSIS ===");

        int i = 18;
        int required = instance.getTrip(i).getNbVehicleNeed();
        int availableOutArcs = 0;

        System.out.println("Trip " + i + " requires " + required + " vehicles");
        System.out.println("Trip " + i + " time window: [" +
                instance.getTrip(i).getEarliestDepartureTime() + ", " +
                instance.getTrip(i).getLatestDepartureTime() + "]");
        ...
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            ...
            if (driverSchedule.whetherTripPresent(i)) {
                ...
                System.out.println("Trip " + i + " found in driver schedule " + d);

                // 找到固定时间
                for (int t = 0; t < driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().size(); t++) {
                    int idTrip = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(t).getIdOfTrip();
                    if (idTrip == i) {
                        int fixedTime = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(t).getDepartureTime();
                        System.out.println("Trip " + i + " fixed time: " + fixedTime);
                    }
                }
            }
        }
        ...
        if (!foundInSchedule) {
            System.err.println("*** PROBLEM: Trip " + i + " NOT found in any driver schedule! ***");
        }

        // 检查可用出弧
        System.out.println("\nAvailable out-arcs for trip " + i + ":");
        ...
        for (int j = 0; j < nbTrip; j++) {
            if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                boolean timeOK = this.whetherPossibleArcWithFixedTime(i, j);
                int connectionTime = this.getFixedConnectionTimeAfterFixedAsDriverSchedule(i, j);

                if (timeOK) {
                    availableOutArcs++;
                    System.out.println("  ✓ To trip " + j + " (connection: " + connectionTime + ")");
                } else {
                    System.out.println("  ✗ To trip " + j + " BLOCKED (connection: " + connectionTime +
                            ", required: " + instance.getMinPlanTurnTime() + ")");
                }
            }
        }

        System.out.println("\nSUMMARY for trip " + i + ":");
        System.out.println("  Required vehicles: " + required);
        System.out.println("  Available out-arcs: " + availableOutArcs);

        if (availableOutArcs < required) {
            System.err.println("  *** INFEASIBLE: Not enough out-arcs! ***");
        }
    }

    private void checkTrip18Variables() throws IloException {
        System.out.println("=== TRIP 18 VARIABLE CHECK (SIMPLE) ===");
        ...
        if (varVehicleArc == null) {
            System.err.println("*** varVehicleArc is NULL! ***");
            return;
        }

        if (varVehicleArc[i] == null) {
            System.err.println("*** varVehicleArc[18] is NULL! ***");
            return;
        }

        System.out.println("Array structure OK");

        // 检查到行程的变量（只显示有问题的）
        for (int j = 0; j < nbTrip; j++) {
            if (instance.whetherHavePossibleArcAfterCleaning(i, j) &&
                    this.whetherPossibleArcWithFixedTime(i, j)) {

                if (varVehicleArc[i][j] == null) {
                    System.err.println("NULL var: [" + i + "][" + j + "]");
                    nullCount++;
                } else {
                    validCount++;
                }
        ...
        if (instance.whetherVehicleCanEndAtTrip(i)) {
            for (int k = 0; k < nbDepot; k++) {
                Depot depot = instance.getDepot(k);
                if (instance.getTrip(i).getIdOfEndCity() == depot.getIdOfCityAsDepot()) {
                    int depotIndex = depot.getIndexOfDepotAsEndingPoint();

                    if (varVehicleArc[i][depotIndex] == null) {
                        System.err.println("NULL depot var: [" + i + "][" + depotIndex + "]");
                        nullCount++;
                    } else {
                        validCount++;
                    }
        ...
        System.out.println("Result: " + validCount + " valid, " + nullCount + " null");
    }

    private void debugVariableCreation()  throws IloException {

            System.out.println("=== DEBUGGING VARIABLE DEFINITION ===");

            System.out.println("whetherSolveRelaxationProblem: " + whetherSolveRelaxationProblem);
            System.out.println("nbNodes: " + nbNodes);
            System.out.println("nbTrip: " + nbTrip);
            System.out.println("nbDepot: " + nbDepot);

            try {
                System.out.println("Step 1: Creating array...");
                varVehicleArc = new IloNumVar[nbNodes][nbNodes];
                System.out.println("✓ Array created, size: " + varVehicleArc.length);

                System.out.println("Step 2: Entering main creation loop...");

                if (this.whetherSolveRelaxationProblem == true) {
                    System.out.println("  Branch: Relaxation problem");
                } else {
                    System.out.println("  Branch: Integer problem");

                    // 测试创建单个变量
                    System.out.println("  Testing single variable creation...");
                    varVehicleArc[0][0] = cplex.intVar(0, nbVehicle, "test_var");
                    System.out.println("  ✓ Test variable created");

                    // 检查行程18的条件
                    int i = 18, j = 2;
                    boolean afterCleaning = instance.whetherHavePossibleArcAfterCleaning(i, j);
                    boolean withFixedTime = this.whetherPossibleArcWithFixedTime(i, j);

                    System.out.println("  Trip 18->2: afterCleaning=" + afterCleaning +
                            ", withFixedTime=" + withFixedTime);

                    if (afterCleaning && withFixedTime) {
                        System.out.println("  Creating variable [18][2]...");
                        varVehicleArc[i][j] = cplex.intVar(0, nbVehicle, "debug_x_v_" + i + "_" + j);
                        System.out.println("  ✓ Variable [18][2] created: " +
                                (varVehicleArc[i][j] != null));
                    }
                }

            } catch (Exception e) {
                System.err.println("*** EXCEPTION in variable definition: " + e.getMessage() + " ***");
                e.printStackTrace();
            }
    ...
    private void testTrip18ConstraintOnly() throws IloException {

            System.out.println("=== TESTING TRIP 18 CONSTRAINT CREATION ===");
            ...
            try {
                ...
                if (instance.whetherVehicleCanEndAtTrip(i)) {
                    for (int k = 0; k < nbDepot; k++) {
                        ...
                        if (instance.getTrip(i).getIdOfEndCity() == depot.getIdOfCityAsDepot()) {
                            if (varVehicleArc[i][indexOfDepotAsEndingPoint] != null) {
                                expr.addTerm(varVehicleArc[i][indexOfDepotAsEndingPoint], 1);
                                termCount++;
                                System.out.println("Added depot term " + termCount);
                            } else {
                                System.err.println("Depot var is null!");
                            }
                        }
                    }
                }
                ...
                for (int j = 0; j < nbTrip; j++) {
                    if (instance.whetherHavePossibleArcAfterCleaning(i, j) &&
                        ...
                        if (varVehicleArc[i][j] != null) {
                            ...
                            termCount++;
                            if (termCount <= 3) { // 只显示前3个
                                System.out.println("Added trip term " + termCount + " (to trip " + j + ")");
                            }
                        } else {
                            System.err.println("Trip " + j + " var is null!");
                        }
                    }
                }
                ...
                if (termCount == 0) {
                    System.err.println("*** NO TERMS IN EXPRESSION! ***");
                    return;
                }

                // 尝试创建约束
                int required = instance.getTrip(i).getNbVehicleNeed();
                System.out.println("Creating constraint: sum = " + required);

                IloRange constraint = cplex.addEq(expr, required, "TestTrip18");
                System.out.println("✓ Constraint created successfully");

                // 尝试导出看看
                cplex.exportModel("trip18_test.lp");
                System.out.println("Model exported to trip18_test.lp");

                // 尝试求解这个简单的情况
                boolean solved = cplex.solve();
                System.out.println("Solve result: " + solved);
                if (!solved) {
                    System.out.println("Status: " + cplex.getStatus());
                }

            } catch (Exception e) {
                System.err.println("*** Exception creating constraint: " + e.getMessage() + " ***");
                e.printStackTrace();
            }
    ...
    private void printSolutionAfterSolve() throws IloException {
        ...
        for (int k = 0; k < instance.getNbDepots(); k++) {
            ...
            if (cplex.getValue(varVehicleSourceDepotArc[k]) > 0.99) {
                // System.out.println("vehicle"  + "_source_" + depot.getIndexOfDepotAsStartingPoint() + "="+cplex.getValue(varVehicleSourceDepotArc[k]));
            }
        ...
        for (int i = 0; i < nbTrip; i++) {
                ...
                for (int k = 0; k < nbDepot; k++) {
                    ...
                    if (instance.getTrip(i).getIdOfStartCity() == depot.getIdOfCityAsDepot()) {
                        if (cplex.getValue(varVehicleArc[depot.getIndexOfDepotAsStartingPoint()][i]) > 0.9999) {
                            System.out.println("Check vehicle" + "_" + depot.getIndexOfDepotAsStartingPoint() + "_" + i + "=" + cplex.getValue(varVehicleArc[depot.getIndexOfDepotAsStartingPoint()][i]));
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < nbTrip; j++) {
                if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    if (this.whetherPossibleArcWithFixedTime(i, j)) {
                        ...
                        if (cplex.getValue(varVehicleArc[i][j]) > 0.999) {
                            System.out.println("Check the number of vehicle_" + "perform arc " + i + "_" + j + "=" + cplex.getValue(varVehicleArc[i][j]));
                        }
...
//        }
                ...
                if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    if (this.whetherPossibleArcWithFixedTime(i, j)) {
                        if (qi == qj && qi == 2) {
                            ...
                            System.out.println("whether one vehicle perform combined to combined connection " + cplex.getValue(varWhetherOneVehiclePerformArcForAllComb[i][j]));
                            System.out.println("whether two vehicles perform combined to combined connection " + cplex.getValue(varWhetherTwoVehiclePerformArcForAllComb[i][j]));
                            System.out.println("whether vehicle flow is consistent perform combined to combined connection " + cplex.getValue(varVehicleArc[i][j]));
                            System.out.println("whether there is a vehicle short connection from combined to combined connection" + cplex.getValue(varWhetherVehicleShortArc[i][j]));
                            System.out.println("whether there the time is casue short connection first trip startT: " + cplex.getValue(varTripStartTime[i]) + " second Trip start:" + cplex.getValue(varTripStartTime[j]));
                        }
                    }
                }
        ...
        //2024.9.17
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            if (cplex.getValue(varStartingTimeOfDriverSchedule[d]) > 0.999) {
                System.out.println("driver schedule_" + d + " start time " + varStartingTimeOfDriverSchedule[d]);
            }
        }

        for (int i = 0; i < nbTrip; i++) {
            if (cplex.getValue(varTripStartTime[i]) >= 0.999999) {
                int start = (int) Math.round(cplex.getValue(varTripStartTime[i]));
                System.out.println("trip_" + i + " start time unit_" + cplex.getValue(varTripStartTime[i]));
                System.out.println("after math round" + start);
            }
        }
            ...
            for (int j = 0; j < nbTrip; j++) {
                if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    if (this.whetherPossibleArcWithFixedTime(i, j)) {
                        ...
                        System.out.println("check maxWaiting with math round" + maxWaitingTime + "compared to not " + instance.getMaxWaitingTime(i, j));
                    ...
                    if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (this.schedules.whetherIsContinuousArc(i, j)) {//2025.
                                ...
                                //(instance.getTrip(j).getLatestDepartureTime()- instance.getTrip(i).getEarliestDepartureTime() - duration_i);
...
//                                    }
        ...
        for (int i = 0; i < nbTrip; i++) {
            for (int j = 0; j < nbTrip; j++) {
                if (this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    for (int d = 0; d < nbDriverValidInSchedules; d++) {
                        DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
                        if (driverSchedule.whetherArcExist(i, j)) {
//                            if(i==24&&j==25){
//                                System.out.println("i==24&&j==25 waiting value "+cplex.getValue(varDriverWaitingTime[d][i][j]) );
//                            }
                            if (cplex.getValue(varDriverWaitingTime[d][i][j]) < instance.getShortConnectionTimeForDriver()) {
                                System.out.println("driver short connection driver_" + d + "_" + i + "_" + j + " by waiting time " + cplex.getValue(varDriverWaitingTime[d][i][j]));
//                                System.out.println("check whether greater than minPlan "+cplex.getValue(varWhetherWaitGreaterThanMinPlan[i][j]));
//                                System.out.println("check whether less than driver short connection "+cplex.getValue(varWhetherWaitLessThanDriverShort[i][j]));
                                System.out.println("check whether short connection by binary " + cplex.getValue(varWhetherDriverShortArc[i][j]));
                            }
//                            if(i==24&&j==25){
//                                System.out.println("i==24&&j==25 detect value "+cplex.getValue(varWhetherDriverShortArc[i][j]));
//                            }
                            if (cplex.getValue(varWhetherDriverShortArc[i][j]) > 0.99) {
                                System.out.println("detect whether short connection trip_" + i + "_" + j);
                            }
                        }
                    }
                }
    ...
    private void cnstFixedTimeAccoringDriverSchedule() throws IloException {
        for (int i = 0; i < nbTrip; i++) {
            for (int d = 0; d < nbDriverValidInSchedules; d++) {
                DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
                if (driverSchedule.whetherTripPresent(i)) {
                    for (int t = 0; t < driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().size(); t++) {
                        int idTrip = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(t).getIdOfTrip();
                        int nbVehcleNeed = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(t).getNbVehicleNeed();
                        if (idTrip == i && nbVehcleNeed == 1) {
                            int startTime = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(t).getDepartureTime();
                            this.varTripStartTime[i].setLB(startTime);
                            this.varTripStartTime[i].setUB(startTime);
                           // System.out.println("set departure time task_i" + i + " with start time " + startTime);
                        }
                    }
        ...
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                if (instance.whetherHavePossibleArcAfterCleaning(j, i)) {
                    if (this.whetherPossibleArcWithFixedTime(j, i)) {
...
//    }
    ...
    private void cnstVehicleWaitingTimeForNotAllCombinedArc_M1() throws IloException {
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                int nbVehicleNeed_j = instance.getTrip(j).getNbVehicleNeed();
                if (this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j) && (nbVehicleNeed_i == 1 || nbVehicleNeed_j == 1)) {
                    int duration_i = instance.getTrip(i).getDuration();
                    int maxWaitingTime = instance.getMaxWaitingTime(i, j);
                    if (maxWaitingTime >= minPlanTime) {
                        int M_Waiting = maxWaitingTime+1;
                        IloLinearNumExpr expr = cplex.linearNumExpr();
        ...
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                ...
                if (this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j) && (nbVehicleNeed_i == 1 || nbVehicleNeed_j == 1)) {
                    ...
                    if (maxWaitingTime >= minPlanTime) {
                        int M_Waiting = maxWaitingTime+1;
                        //duration_i + instance.getTrip(i).getLatestDepartureTime() - instance.getTrip(j).getEarliestDepartureTime() + 1;
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varVehicleWaitingTimeForNotAllComb[i][j], 1);
                        expr.addTerm(varTripStartTime[j], -1);
                        expr.addTerm(varTripStartTime[i], 1);
    ...
    private void cnstOneVehicleWaitingTimeForAllCombinedArc_M1() throws IloException {
        ...
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                int nbVehicleNeed_j = instance.getTrip(j).getNbVehicleNeed();
                if ((nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) && this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    int duration_i = instance.getTrip(i).getDuration();
                    int maxWaitingTime = instance.getMaxWaitingTime(i, j);
                    if (maxWaitingTime >= minPlanTime) {
                        int M_Waiting = maxWaitingTime+1;
            ...
            for (int j = 0; j < nbTrip; j++) {
                ...
                if ((nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) && this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    ...
                    if (maxWaitingTime >= minPlanTime) {
                        ...
                        IloConstraint constraint = cplex.addGe(expr, -1 * duration_i - M_Waiting, "cnstVehicleWaitingTimeForAllCombinedArc_M1_" + i + "_" + j);
        ...
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                ...
                if ((nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) && this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    ...
                    if (maxWaitingTime >= minPlanTime) {
                        int M_Waiting = maxWaitingTime+1;
                        //2 * (duration_i + instance.getTrip(i).getLatestDepartureTime() - instance.getTrip(j).getEarliestDepartureTime() + 1);
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varTwoVehicleWaitingTimeForAllComb[i][j], 1);
                        expr.addTerm(varTripStartTime[j], -1);
                        expr.addTerm(varTripStartTime[i], 1);
                        expr.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], M_Waiting);
                        IloConstraint constraint = cplex.addLe(expr, -1 * duration_i + M_Waiting, "cnstVehicleWaitingTimeForAllCombinedArc_M2_" + i + "_" + j);
                    }
                }
            }
        }
            ...
            for (int j = 0; j < nbTrip; j++) {
                ...
                if ((nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) && this.whetherPossibleArcWithFixedTime(i, j) && instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                    ...
                    if (maxWaitingTime >= minPlanTime) {
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varTwoVehicleWaitingTimeForAllComb[i][j], 1);
                        expr.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], -minPlanTime);
    ...
    private void cnstTwoVehicleWaitingTimeForAllCombinedArc_2() throws IloException {
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                int nbVehicleNeed_j = instance.getTrip(j).getNbVehicleNeed();
                if ((nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) && instance.whetherHavePossibleArcAfterCleaning(i, j) && this.whetherPossibleArcWithFixedTime(i, j)) {
                    int duration_i = instance.getTrip(i).getDuration();
                    int maxWaitingTime = instance.getMaxWaitingTime(i, j);
                    if (maxWaitingTime >= minPlanTime) {
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varTwoVehicleWaitingTimeForAllComb[i][j], 1);
                        expr.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], -maxWaitingTime);
                        ...
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (driverSchedule.whetherArcExist(i, j)) {//2025.1.15
                                ...
                                if (maxWaitingTime >= minPlanTime) {
                                    ...
                                    expr.addTerm(varTripStartTime[j], -1);
                                    expr.addTerm(varTripStartTime[i], 1);
                                    IloConstraint constraint = cplex.addGe(expr, -duration_i, "cnstDriverWaitingTime_M1_" + i + "_" + j);
                        ...
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (driverSchedule.whetherArcExist(i, j)) {//2025.1.15
                                ...
                                if (maxWaitingTime >= minPlanTime) {
                                    ...
                                    expr.addTerm(varTripStartTime[j], -1);
                                    expr.addTerm(varTripStartTime[i], 1);
                                    IloConstraint constraint = cplex.addLe(expr, -duration_i, "cnstDriverWaitingTime_M2_" + i + "_" + j);
                                }
                            }
                        }
                    }
                        ...
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (driverSchedule.whetherArcExist(i, j)) {//2025.1.15
                                ...
                                if (maxWaitingTime >= minPlanTime) {
                                    ...
                                    IloConstraint constraint = cplex.addGe(expr, minPlanTime, "cnstDriverWaitingTime_1_" + i + "_" + j);
                                }
                            }
                        }
                    }
                        ...
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (driverSchedule.whetherArcExist(i, j)) {
                                ...
                                if (maxWaitingTime >= minPlanTime) {
                                    IloLinearNumExpr expr = cplex.linearNumExpr();
                                    expr.addTerm(varDriverWaitingTime[d][i][j], 1);
                                    IloConstraint constraint = cplex.addLe(expr, maxWaitingTime, "cnstDriverWaitingTime_2_" + i + "_" + j);
                                }
                            }
                        }
                    }
    ...
    private void cnstDetectOneVehicleShortConnectionArcForAllCombined() throws IloException {
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                int nbVehicleNeed_j = instance.getTrip(j).getNbVehicleNeed();
                if (instance.whetherHavePossibleArcAfterCleaning(i, j) && this.whetherPossibleArcWithFixedTime(i, j) && nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) {
                    int vehicleShortTime = instance.getShortConnectionTimeForVehicle();
                    int diffValue = instance.getMinPlanTurnTime() - vehicleShortTime;
                    IloLinearNumExpr expr = cplex.linearNumExpr();
                    if (varOneVehicleWaitingTimeForAllComb[i][j] == null) {
                        System.out.println("ERROR" + "varOneVehicleWaitingTimeForAllComb[i][j]" + i + "_" + j);
                    }
                    if (varWhetherOneVehiclePerformArcForAllComb[i][j] == null) {
                        System.out.println("ERROR" + "varWhetherOneVehiclePerformArcForAllComb[i][j]" + i + "_" + j);
                    }

                    if (varWhetherVehicleShortArc[i][j] == null) {
                        System.out.println("ERROR" + "varWhetherVehicleShortArc[i][j]" + i + "_" + j);
                    }
                    expr.addTerm(varOneVehicleWaitingTimeForAllComb[i][j], 1);
    ...
    // new for two vehicles perform the combined arcs (40)
        ...
        for (int i = 0; i < nbTrip; i++) {
            ...
            for (int j = 0; j < nbTrip; j++) {
                ...
                if (instance.whetherHavePossibleArcAfterCleaning(i, j) && this.whetherPossibleArcWithFixedTime(i, j) && nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) {
                    ...
                    IloLinearNumExpr expr = cplex.linearNumExpr();
                    expr.addTerm(varTwoVehicleWaitingTimeForAllComb[i][j], 1);
                    expr.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], -vehicleShortTime);
                    expr.addTerm(varWhetherVehicleShortArc[i][j], -diffValue);
                    IloConstraint constraint = cplex.addGe(expr, 0, "DetectVehicleShortConnection_" + "_" + i + "_" + j);
                }
            }
    ...
    private void cnstLinkWhetherVehicleShortConnectionWhetherTwoPerformArc() throws IloException {// new add 2025.2.4
        for (int i = 0; i < nbTrip; i++) {
            int nbVehicleNeed_i = instance.getTrip(i).getNbVehicleNeed();
            for (int j = 0; j < nbTrip; j++) {
                int nbVehicleNeed_j = instance.getTrip(j).getNbVehicleNeed();
                if (instance.whetherHavePossibleArcAfterCleaning(i, j) && this.whetherPossibleArcWithFixedTime(i, j) && nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) {
                    ...
                    expr.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], -1);
                    IloConstraint constraint = cplex.addLe(expr, 0, "link vehicleArc Between combined task_" + i + "_" + j);
                    ...
                    if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            if (driverSchedule.whetherArcExist(i, j)) {//2025.1.15
                                ...
                                int diffValue = driverShortConnectionTime - instance.getMinPlanTurnTime();
                                expr.addTerm(varWhetherDriverShortArc[i][j], diffValue);
                                expr.addTerm(varDriverWaitingTime[d][i][j], 1);
                                IloConstraint constraint = cplex.addGe(expr, driverShortConnectionTime, "DetectDriverShortConnectionTime_" + d + "_" + i + "_" + j);
                            }
                        }
                    }
                }
...
//    private void cnstDetectDriverShortConnectionArc2() throws IloException {
//        for (int d = 0; d < nbDriverValidInSchedules; d++) {
//            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
//            for (int i = 0; i < nbTrip; i++) {
//                for (int j = 0; j < nbTrip; j++) {
...
//                        if (driverSchedule.whetherArcExist(i, j)) {//2025.1.15
//                            IloLinearNumExpr expr = cplex.linearNumExpr();
//                            int M=instance.getMaxWaitingTime(i,j);
//                            int driverShortConnectionTime = instance.getShortConnectionTimeForDriver();
//                            expr.addTerm(varWhetherDriverShortArc[i][j], M);
//                            expr.addTerm(varDriverWaitingTime[d][i][j], 1);
//                            IloConstraint constraint = cplex.addLe(expr, M+driverShortConnectionTime, "DetectDriverShortConnectionTime2_" + d + "_" + i + "_" + j);
//
//                        }
//                    }
//                }
...
//                                expr.addTerm(this.varWhetherWaitGreaterThanMinPlan[i][j], M);
//                                IloRange constraintsShortConnection = cplex.addGe(expr, 1-minPlan, "idleTimeWhetherGreaterThanMinP1");
//                            }
//                        }
...
//                            expr.addTerm(this.varDriverWaitingTime[d][i][j], -1);
//                            expr.addTerm(this.varWhetherWaitGreaterThanMinPlan[i][j], M);
//                            IloRange constraintsShortConnection = cplex.addLe(expr, M-minPlan, "idleTimeWhetherGreaterThanMinP2");
//                        }
//                    }
//                }
//
...
//                                IloLinearNumExpr expr = cplex.linearNumExpr();
//                                expr.addTerm(this.varDriverWaitingTime[d][i][j], -1);
//                                expr.addTerm(this.varWhetherWaitLessThanDriverShort[i][j], -M);
//                                IloRange constraintsShortConnection = cplex.addLe(expr, -(TDS + 1), "cnstWhetherLessThanDriverShort1");
//                            }
//                        }
//                    }
//                }
...
//                                expr.addTerm(this.varDriverWaitingTime[d][i][j], -1);
//                                expr.addTerm(this.varWhetherWaitLessThanDriverShort[i][j], -M);
//                                IloRange constraintsShortConnection = cplex.addGe(expr, -M-TDS, "cnstWhetherLessThanDriverShort1");
//                            }
//                        }
//                    }
//                }
...
//                    if (minWaitingTime <= instance.getShortConnectionTimeForDriver()) {
//                        IloLinearNumExpr expr = cplex.linearNumExpr();
//                        expr.addTerm(this.varWhetherWaitGreaterThanMinPlan[i][j], -1);
...
//                    if (minWaitingTime <= instance.getShortConnectionTimeForDriver()) {
//                        IloLinearNumExpr expr = cplex.linearNumExpr();
//                        expr.addTerm(this.varWhetherWaitLessThanDriverShort[i][j], -1);
//                        expr.addTerm(this.varWhetherDriverShortArc[i][j], 1);
//                        IloRange constraintsShortConnection = cplex.addLe(expr,0, "cnstWhetherLessThanDriverShort1");
//                    }
//                }
...
//    }
    ...
    private void cnstStartingTimeOfSchedule() throws IloException {
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
            Trip firstTrip = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(0);
            for (int p = 0; p < nbDepot; p++) {
                int idCityAsDepot = instance.getDepot(p).getIdOfCityAsDepot();
                if (instance.whetherDriverCanStartWithTrip(firstTrip.getIdOfTrip())) {
                    int idStartCityOfTrip = instance.getTrip(firstTrip.getIdOfTrip()).getIdOfStartCity();
                    if (idCityAsDepot == idStartCityOfTrip) {
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varStartingTimeOfDriverSchedule[d], 1);
                        expr.addTerm(varTripStartTime[firstTrip.getIdOfTrip()], -1);
                        IloConstraint constraint = cplex.addLe(expr, 0, "startScheduleDriver_" + d);
                    }
                }
    ...
    private void cnstEndingTimeOfSchedule() throws IloException {
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
            int nbTripPerformedByDriver = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().size();
            Trip lastTipInSchedule = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(nbTripPerformedByDriver - 1);
            System.out.println("last trip" + lastTipInSchedule);
            for (int p = 0; p < nbDepot; p++) {
                int idCityAsDepot = instance.getDepot(p).getIdOfCityAsDepot();
                // System.out.println("instance.whetherDriverCanEndAtTrip(lastTipInSchedule.getIdOfTrip())"+instance.whetherDriverCanEndAtTrip(lastTipInSchedule.getIdOfTrip()));
//                for (int i = 0; i < nbTrip; i++) {
                if (instance.whetherDriverCanEndAtTrip(lastTipInSchedule.getIdOfTrip())) {
                    int idEndingCityOfTrip = lastTipInSchedule.getIdOfEndCity();
                    int duration_i = instance.getTrip(lastTipInSchedule.getIdOfTrip()).getDuration();
                    if (idCityAsDepot == idEndingCityOfTrip) {
                        IloLinearNumExpr expr = cplex.linearNumExpr();
                        expr.addTerm(varEndingTimeOfDriverSchedule[d], 1);
                        expr.addTerm(varTripStartTime[lastTipInSchedule.getIdOfTrip()], -1);
                        IloConstraint constraint = cplex.addGe(expr, duration_i, "EndScheduleDriver_" + d);
                    }
                }
//                }
            }
    ...
    private void cnstChangeVehicleBetweenNormalTrips() throws IloException {
        for (int d = 0; d < this.schedules.getDriverSchedules().size(); d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
            for (int i = 0; i < nbTrip; i++) {
                for (int j = 0; j < nbTrip; j++) {
                    if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
    ...
    private void cnstOnePersonDriving() throws IloException {
        for (int i = 0; i < nbTrip; i++) {
            IloLinearNumExpr expr = cplex.linearNumExpr();
            for (int d = 0; d < this.schedules.getDriverSchedules().size(); d++) {
                DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
                    ...
                    if (this.whetherPossibleArcWithFixedTime(i, j)) {
                        ...
                        if (nbVehicleNeed_i == 2 && nbVehicleNeed_j == 2) {
                            if (maxWaitingTime >= minPlanTime) {
                                ...
                                obj.addTerm(varWhetherTwoVehiclePerformArcForAllComb[i][j], 2 * idleTimeCost_v);
                            }
                        }
                    }
                }
...
//        varWhetherWaitGreaterThanMinPlan= new IloNumVar[nbTrip][nbTrip];//2025.2.5
//        varWhetherWaitLessThanDriverShort= new IloNumVar[nbTrip][nbTrip];//2025.2.5
...
//        varLeading = new IloNumVar[nbVehicle][nbTrip]; //remove
        ...
        if (this.whetherSolveRelaxationProblem == true) {
            ...
            for (int d = 0; d < nbDriver; d++) {
                for (int i = 0; i < nbTrip; i++) {
                    varDriving[d][i] = cplex.numVar(0, 1, "o^d_" + d + "_" + i);
                }
            }
            ...
            for (int d = 0; d < nbDriver; d++) {
                for (int i = 0; i < nbTrip; i++) {
                    for (int j = 0; j < nbTrip; j++) {
                        varChangeOver[d][i][j] = cplex.numVar(0, 1, "y^d_" + d + "_" + i + "_" + j);
                    }
                }
            }
                    ...
                    if (instance.whetherHavePossibleArcAfterCleaning(i, j)) {
                        if (this.whetherPossibleArcWithFixedTime(i, j)) {
                            ...
                            if (maxWaitingTime >= minPlanTime) {
                               ...
                               // System.out.println("initialize OneVehicleWaitingTimeForAllComb " + i + "_" + j);
...
//            for (int i = 0; i < nbTrip; i++) {
//                for (int j = 0; j < nbTrip; j++) {
//                    varWhetherWaitGreaterThanMinPlan[i][j] = cplex.boolVar("b^DS1_" + i + "_" + j);
//                }
//            }
//
//            for (int i = 0; i < nbTrip; i++) {
//                for (int j = 0; j < nbTrip; j++) {
//                    varWhetherWaitLessThanDriverShort[i][j] = cplex.boolVar("b^DS2_" + i + "_" + j);
//                }
//            }
            ...
            for (int d = 0; d < nbDriver; d++) {
                for (int i = 0; i < nbTrip; i++) {
                    varDriving[d][i] = cplex.boolVar("o^d_" + d + "_" + i);
                }
            }
            ...
            for (int d = 0; d < nbDriver; d++) {
                for (int i = 0; i < nbTrip; i++) {
                    for (int j = 0; j < nbTrip; j++) {
                        varChangeOver[d][i][j] = cplex.boolVar("y^d_" + d + "_" + i + "_" + j);
                    }
                }
            }
            //check why
    ...
    private Solution getSolutionObtainedByFlowMatrixInformation() throws IloException {
        Solution solution = new Solution(instance);
        //give vehicle path from the cplex solution
        Routes routes = this.getVehicleRoutesFromCplex();
        System.out.println("Check routes from cplex by martrix" + routes);
        for (int r = 0; r < routes.getRoutes().size(); r++) {
            System.out.println("check routes size " + routes.getRoutes().size());
...
//        System.out.println("capacityMatrix" + initializeResidualCapacityMatrixForAllTrips());
    ...
    private void generateRoutesFromStartingDepot(Depot startingDepot, int[][] CapacityMatrix, Routes allVehicleRoutes) throws IloException {
        ...
        System.out.println("Check generate route from starting depot " + startingDepot + ": " + routes);
    }
...
// Helper method to generate routes from a trip
                ...
                for (int l = 0; l < nbNodes; l++) {
                    if (CapacityMatrix[idCurrentNode][l] >= 1) {
                        if (l < nbTrip) {
                            ...
                            if (l == 2) {
                                System.out.println("write solution part " + startTimeOfTrip);
                            }
    ...
    public int getFixedConnectionTimeAfterFixedAsDriverSchedule(int i, int j) {
        int connectionTimeAfterFixed = Integer.MAX_VALUE;
        int depI = Integer.MAX_VALUE;
        int depJ = Integer.MAX_VALUE;
        int duration = Integer.MAX_VALUE;
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
            for (int index = 0; index < driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().size(); index++) {
                int idTrip = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(index).getIdOfTrip();
                if (idTrip == i) {
                    depI = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(index).getDepartureTime();
                    duration = instance.getTrip(i).getDuration();
                }
                if (idTrip == j) {
                    depJ = driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().get(index).getDepartureTime();
                }
            }
        }
        // 确保两个行程都被找到
        if (depI == Integer.MAX_VALUE || depJ == Integer.MAX_VALUE) {
            return Integer.MAX_VALUE; // 或者抛出异常
        }
        connectionTimeAfterFixed = depJ - depI - duration;
        return connectionTimeAfterFixed;
    }


    private boolean whetherPossibleArcWithFixedTime(int i, int j) {
        return this.getFixedConnectionTimeAfterFixedAsDriverSchedule(i, j) >= instance.getMinPlanTurnTime();
    }
    ...
    public static void main(String[] args) throws IloException, IOException {
        InstanceReader reader1 = new InstanceReader("inst_nbCity03_Size90_Day1_nbTrips035_combPer0.25_TW6.txt");
        Instance instance = reader1.readFile();
        System.out.println(instance);

        SchedulesReader reader2 = new SchedulesReader("scheduleSolution_inst_nbCity03_Size90_Day1_nbTrips035_combPer0.25_TW6.txt", instance);
        Schedules schedules1 = reader2.readFile();

        SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost solverWithFormulation1 =
                new SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost(instance, schedules1, false);

// 1. 首先分析Trip 18的基本问题
        solverWithFormulation1.analyzeTrip18Problem();
