...
public class PathForVehicle {
    ...
    public void setVehiclePathFromRoute(Route route, int idOfVehicle){
        int nbTripInRoute=route.getNbTripsPerformedByVehicle();
        for(int i=0;i<nbTripInRoute;i++){
            TripWithStartingInfos tripWithStartingInfos = route.getNodes().get(i+1).getTripWithStartInfos();
...
//                        System.out.println("now the idle time in vehicle_"+idOfVehicle+" path between trip "+idOfFormerTrip+" to trip " +idOfLatterTrip+" is"+connectionTime);
//                        System.out.println("now the idle time cost until trip " +idOfLatterTrip+" is"+cost);
     ...
     */
    public boolean areConsecutiveInPathOfVehicle(Trip lFirst, Trip lSecond) {//modify1108
//        for (int i = 0; i < this.vehiclePath.size() - 1; i++) {
//
...
//
//        return false;
        boolean whetherConsective=false;
        int idF=lFirst.getIdOfTrip();
        int idS=lSecond.getIdOfTrip();
        for (int i = 0; i < this.vehiclePath.size() - 1; i++) {
            int currentIdF=this.vehiclePath.get(i).getIdOfTrip();
            int currentIdS=this.vehiclePath.get(i+1).getIdOfTrip();
            if (currentIdF==idF && currentIdS==idS) {
                whetherConsective=true;
                break;
            }
        }
        return whetherConsective;
    }
    ...
    public int getIdStartDepot(){
        int idStartDepot=Integer.MAX_VALUE;
        TripWithStartingInfos tripWithStartingInfos=this.getFirstTripWithStartingInfos();
        int idStartCityAsFirstTrip=tripWithStartingInfos.getIdOfStartCity();
        for(int k=0;k<instance.getNbDepots();k++){
            Depot depot=instance.getDepot(k);
            int idCityAsDepot=depot.getIdOfCityAsDepot();
            if(idCityAsDepot==idStartCityAsFirstTrip){
                idStartDepot=depot.getIdOfDepot();
            }
        }
        return idStartDepot;
    }

    public int getIdEndDepot(){
        int idEndDepot=Integer.MAX_VALUE;

        TripWithStartingInfos tripWithStartingInfos=this.getLastTripWithStartingInfos();
        int idEndCityAsLastTrip=tripWithStartingInfos.getIdOfEndCity();

        for(int k=0;k<instance.getNbDepots();k++){
            Depot depot=instance.getDepot(k);
            int idCityAsDepot=depot.getIdOfCityAsDepot();
            if(idCityAsDepot==idEndCityAsLastTrip){
                idEndDepot=depot.getIdOfDepot();
            }
        }
        return idEndDepot;
    ...
    public String toString() {
        ...
        String s="vehicle "+idOfVehicle+ " "+vehiclePath.size()+" "+this.getIdStartDepot();
        for(int i=0;i<vehiclePath.size();i++){
            s+=" "+vehiclePath.get(i).getIdOfTrip()
                    +" "+vehiclePath.get(i).getStartingTime()
            ;
        }
        s+=" "+this.getIdEndDepot();
        return s;
    }
