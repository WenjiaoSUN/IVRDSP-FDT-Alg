...
public class SolverWithFormulation_1 {
    ...
    private String solutionWithNoTimeWindowPath;
    //solutionWithNoTimeWindowPath;
...
//
    ...
    public SolverWithFormulation_1(Instance instance) throws IloException {
        ...
        this.solutionWithNoTimeWindowPath = null;
    ...
    public SolverWithFormulation_1(Instance instance, String solutionWithNoTimeWindowPath, Boolean whetherSolveRelaxationProblem) throws IloException {
        this(instance);
        this.solutionWithNoTimeWindowPath = solutionWithNoTimeWindowPath;
    ...
    public Solution solveWithCplex() {
        try {
            ...
            cplex.setParam(IloCplex.Param.TimeLimit, 600);// limit the time is two hour
...
//            if (this.solutionWithNoTimeWindowPath != null && this.whetherSolveRelaxationProblem == false) {
//                addWarmStartBeforeSolve(solutionWithNoTimeWindowPath);
//            }
...
//        //_________________________________________________________________________________________this is for source and sink for the driver
    ...
    private void setValueOfDecisionVariables() throws IloException {
        if (solutionWithNoTimeWindowPath != null) {
            File solutionWithNoTimeWindow = new File(solutionWithNoTimeWindowPath);
            try {
                FileReader fileReader = new FileReader(solutionWithNoTimeWindow);
                BufferedReader bufferedReader = new BufferedReader(fileReader);
                ...
                while (line != null && line.startsWith("vehicle")) {
                    String[] values;
                    values = line.split(" ");
                    int nbTripInPath=Integer.valueOf(values[2]);
                    int m = 3;
                    for(int t=0;t<nbTripInPath-1;t++) {
                        int idOfVehicle=Integer.valueOf(values[1]);
                        System.out.println("i="+Integer.valueOf(values[m]));
                        System.out.println("j="+Integer.valueOf(values[m + 2]));
                        this.varVehicleArc[idOfVehicle][Integer.valueOf(values[m])][Integer.valueOf(values[m + 2])].setLB(1);
                        System.out.println("vehicle arc set information: "+varVehicleArc[idOfVehicle][Integer.valueOf(values[m])][Integer.valueOf(values[m + 2])]);
                        this.varVehicleArc[idOfVehicle][Integer.valueOf(values[m])][Integer.valueOf(values[m + 2])].setUB(1);
                        m=m+2;
                    }
                    line = bufferedReader.readLine();
                ...
                while (line != null && line.startsWith("driver")) {
                    String[] values;
                    values = line.split(" ");
                    System.out.println("now the current line is" +line);
                    int nbTripInPath = Integer.valueOf(values[2]);
                    int m = 3;
                    int idOfDriver = Integer.valueOf(values[1]);
                    for (int t = 0; t < nbTripInPath - 1; t++) {
                        this.varDriverArc[idOfDriver][Integer.valueOf(values[m])][Integer.valueOf(values[m + 3])].setLB(1);
                        System.out.println("driver arc set information id_ "+idOfDriver+" arc " + varDriverArc[idOfDriver][Integer.valueOf(values[m])][Integer.valueOf(values[m + 3])]);
                        this.varDriverArc[idOfDriver][Integer.valueOf(values[m])][Integer.valueOf(values[m + 3])].setUB(1);

                        boolean weatherDriverIsDrivingInFirstTask=Boolean.parseBoolean(values[m + 2]);
                        if(weatherDriverIsDrivingInFirstTask) {
                            this.varDriving[idOfDriver][Integer.valueOf(values[m])].setLB(1);
                            this.varDriving[idOfDriver][Integer.valueOf(values[m])].setUB(1);
                        }else {
                            this.varDriving[idOfDriver][Integer.valueOf(values[m])].setLB(0);
                            this.varDriving[idOfDriver][Integer.valueOf(values[m])].setUB(0);

                        }

                        boolean weatherDriverIsDrivingInSecondTask=Boolean.parseBoolean(values[m + 5]);
                        if(weatherDriverIsDrivingInSecondTask) {
                            this.varDriving[idOfDriver][Integer.valueOf(values[m+3])].setLB(1);
                            this.varDriving[idOfDriver][Integer.valueOf(values[m+3])].setUB(1);
                        }else {
                            this.varDriving[idOfDriver][Integer.valueOf(values[m+3])].setLB(0);
                            this.varDriving[idOfDriver][Integer.valueOf(values[m+3])].setUB(0);
...
//        InstanceReader reader = new InstanceReader("largerExample.txt");
        InstanceReader reader1 = new InstanceReader("inst_nbCity03_Size90_Day1_nbTrips030_combPer0.25_TW2.txt");

        Instance instance = reader1.readFile(); //这个语句将文本的内容就读出来了
        System.out.println(instance);

        String warmStartFileName = "feaSol_nbCity05_Size180_Day1_nbTrips025_combPer0.0_TW0.txt";


        SolverWithFormulation_1 solverWithFormulation1 = new SolverWithFormulation_1(instance, "result_inst_nbCity03_Size90_Day1_nbTrips035_combPer0.25_TW0.txt", false);

        // print solution
        Solution sol = solverWithFormulation1.solveWithCplex();
        if (sol != null) {
                sol.printInfile("sol_inst_nbCity03_Size90_Day1_nbTrips035_combPer0.25_TW0.txt");
                int variableCost= (int) (sol.getTotalIdleTimeCost()+sol.getTotalChangeoverCostForAllDriver());
            System.out.println("Total variableCost is "+variableCost);
            int changeoverCost= (int) sol.getTotalChangeoverCostForAllDriver();
            System.out.println("Total changeoverCost is "+changeoverCost);
            int idlForDriver= (int) sol.getTotalIdleTimeCostOfAllDriver();
            System.out.println("Total idleTimeCotForDriver: "+idlForDriver);
            int idlForVehicle= (int) sol.getTotalIdleTimeCostOfAllVehicle();
            System.out.println("Total idleTimeCostForVehicle: "+idlForVehicle);
        }
