...
//
    ...
    public SolverWithFormulationBasedOnDriverSchedule(Instance instance, Schedules schedules) throws IloException {
    ...
    public SolverWithFormulationBasedOnDriverSchedule(Instance instance, Schedules schedules, Boolean whetherSolveRelaxationProblem) throws IloException {
    ...
    public Solution solveWithCplexBasedOnGivenSchedules() {
        try {
            ...
            //Extra part 3 中的 vehicle short connection part:
            cnstDetectVehicleShortConnectionArc();//(40)
            cnstKeepSameVehiclesDuringShortConnectionInCombinedArc();//(41)

            //Extra part4: vehicle driver and time : driver short connection
...
//
    ...
    private void cnstChangeVehicleFromCombinedToCombinedTripsCorrespondingVerLeadingOrder() throws IloException {
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
            for (int v = 0; v < nbVehicle; v++) {
                for (int i = 0; i < nbTrip; i++) {
...
//        //_________________________________________________________________________________________this is for source and sink for the driver
    ...
    private void setVarDriverArc() throws IloException {
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            DriverSchedule driverSchedule = this.schedules.getDriverSchedules().get(d);
    ...
    private Solution getSolutionFromCplex() throws IloException {
        ...
        for (int d = 0; d < nbDriverValidInSchedules; d++) {
            ...
            for (int l = 0; l < driverSchedule.getTripWithWorkingStatusAndDepartureTimeArrayList().size(); l++) {
                ...
                if (cplex.getValue(varTripStartTime[idTrip]) > 0.999) {
...
//        InstanceReader reader = new InstanceReader("largerExample.txt");
        ...
        String warmStartFileName = "feaSol_nbCity03_Size90_Day1_nbTrips025_combPer0.25_TW3.txt";
        ...
        SolverWithFormulationBasedOnDriverSchedule solverWithFormulation1 = new SolverWithFormulationBasedOnDriverSchedule(instance, schedules1, false);
