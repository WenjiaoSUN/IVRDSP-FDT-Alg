...
public class Benchmark {
    ...
    private static String inputInformsDirectory = "input"; //in order to read the schedules obtained from the first phase
...
//    private static String statisticsFile="statistics.csv";// here is to give a statistics result on the solution
    ...
    private static String solutionDirectory = "initialSolInforms";// in order to put the solutions in this direction
    ...
    public static void read(String[] args) {
        ...
        if (options.containsKey("-input")) {
            inputInformsDirectory = options.get("-input");
        }
    ...
    public static void solveFromFiles() throws IloException, IOException {
        ...
        if (f.isDirectory()) {
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                ...
                String inputFileName = "MergedSolutionArcTimeInforms_" + fileName;// this is for read the schedules obtained from CG
                String inputFilePath = inputInformsDirectory + "\\" + inputFileName;
...
//                String solutionFileName = "solution_" + fileName;
                String solutionFileName = "initialSol_" + fileName;//
                ...
                try {
                    ...
                    if (fileName.startsWith("inst_") && fileName.endsWith(".txt")) {
                        ...
                        InputInformsReader inputInformsReader = new InputInformsReader(inputFilePath, instance);
...
////                        //the following two line is when we solve with no warm file and also no consider changeoverCost
                        SolverWithFormulationBasedOnDriverSchedule solverBasedOnDriverSchedule = new SolverWithFormulationBasedOnDriverSchedule(instance, inputInforms, whetherSolveRelaxationProblem);
                                    ...
                                    for(int t=0;t<instance.getNbTrips();t++){
                                        if(instance.getTrip(t).getNbVehicleNeed()==2){
                                            ...
                                            for(int s=0; s<sol.getPathForDrivers().size(); s++){
                                               ...
                                               for(int p = 0; p<pathForDriver.getTripWithDriveInfosArrayList().size(); p++){
                                                  if(pathForDriver.getTripWithDriveInfosArrayList().get(p).getIdOfTrip()==idCombineTrip&&pathForDriver.getTripWithDriveInfosArrayList().get(p).getDrivingStatus()==true){
                                                      int idOfLeadingVehicle=pathForDriver.getTripWithDriveInfosArrayList().get(p).getIdOfVehicle();//in combined trip driving driver using leading vehicle
