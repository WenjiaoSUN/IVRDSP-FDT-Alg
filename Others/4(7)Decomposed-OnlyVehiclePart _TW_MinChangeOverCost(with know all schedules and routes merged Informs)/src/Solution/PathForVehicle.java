...
public class PathForVehicle {
    ...
    public void setVehiclePathFromRoute(VehicleRoute route, int idOfVehicle){
        int nbTripInRoute=route.getTripWithStartingInfosArrayList().size();
        for(int i=0;i<nbTripInRoute;i++){
            TripWithStartingInfos tripWithStartingInfos =route.getTripWithStartingInfosArrayList().get(i);
...
//                        System.out.println("now the idle time cost until trip " +idOfLatterTrip+" is"+cost);
    ...
    public String toString() {
        ...
        String s="vehicle "+idOfVehicle+ " "+vehiclePath.size();
        for(int i=0;i<vehiclePath.size();i++){
            s+=" "+vehiclePath.get(i).getIdOfTrip()+" "+vehiclePath.get(i).getStartingTime();
        }
        return s;
    }
