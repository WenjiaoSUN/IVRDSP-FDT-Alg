...
//        this.tripWithDriveInfos =new TripWithDriveInfos[nbTrips];
    ...
    public int getMaxNbPossibleArcs() {
        int maxNbArc = 0;
        for (int i = 0; i < nbTrips; i++) {
            for (int j = 0; j < nbTrips; j++) {
        ...
        System.out.println("nbArcForDepot" + nbArcForDepot);
        return maxNbArc;
    }
    ...
    public int getCostForChangeOver(int idOfFirstLeg, int idOfSecondLeg) {
    ...
    public boolean whetherDriverCanStartWithTrip(int idOfDriver, int idOfTrip) {
        boolean whetherDriverCanStartWithTrip = false;
        Driver driver = this.getDriver(idOfDriver);
        for (int k = 0; k < nbDepot; k++) {
            Depot depot = this.getDepot(k);
            Trip trip = this.getTrip(idOfTrip);
            if (trip.getIdOfStartCity() == depot.getIdOfCityAsDepot()) {
    ...
    public boolean whetherDriverCanEndAtTrip(int idOfDriver, int idOfTrip) {
    ...
    public int getMaxWaitingTime(int idFirstTrip, int idSecondTrip) {
        ...
        if (this.whetherHavePossibleArcAfterCleaning(idFirstTrip, idSecondTrip)) {
             maxWaitingTime = this.getTrip(idSecondTrip).getLatestDepartureTime()
    ...
    public int getMinWaitingTime(int idFirstTrip, int idSecondTrip) {
        int minWaitingTime = 0; //infeasible
        if (this.whetherHavePossibleArcAfterCleaning(idFirstTrip, idSecondTrip)) {
            ...
            if (minConTime >= this.minPlanTurnTime) {
...
//    public boolean whetherHavePossibleArc(int idOfFirstTrip, int idOfSecondTrip) {
...
//            int largestConTime = this.trips[idOfSecondTrip].getLatestDepartureTime() - this.trips[idOfFirstTrip].getEarliestDepartureTime()
//                    - this.getDuration(this.trips[idOfFirstTrip].getIdOfStartCity(), this.trips[idOfFirstTrip].getIdOfEndCity());
//            int minConTime = this.trips[idOfSecondTrip].getEarliestDepartureTime() - this.trips[idOfFirstTrip].getLatestDepartureTime()
//                    - this.getDuration(this.trips[idOfFirstTrip].getIdOfStartCity(), this.trips[idOfFirstTrip].getIdOfEndCity());
//
//            if (largestConTime >= this.minPlanTurnTime && largestConTime <= this.maxWorkingTime) {
...
//                            System.out.println("check" + totalDuration);
    ...
    public String toString() {
        String s = "Instance{" + "\n" + "\t" + "instanceName: " + nameOfInstance +
                ...
                + "\n \t" + "maxDrivingTime:" + maxDrivingTime + "\t" + "maxWorkingTime:" + maxWorkingTime + "\t" + "maxPercentageOfCombinedTrip:" + maxPercentageOfCombinedTrip + "\t" + "maxFolderOfTimeSlotAsTimeWindow" + maxFolderOfTimeSlotAsTimeWindow + "\t" + "maxWaitingTime:" + maxPlanTime
