...
//
//    private int nbVehicleInSolution;
//
//    private int nbDriverInSolution;

//     this.fixedCost = this.getSolutionFromCplex().getTotalFixedCost();
//                this.arcCost = this.getSolutionFromCplex().getTotalIdleTimeCost();
//                this.changeOverCost = this.upperBound - this.fixedCost - this.arcCost;

//                this.nbDriverInSolution = (int) (this.getSolutionFromCplex().getFixedCostForDriver() / this.instance.getFixedCostForDriver());
//                this.nbVehicleInSolution = (int) (this.getSolutionFromCplex().getFixedCostForVehicle() / this.instance.getFixedCostForVehicle());
    ...
    public double getTotalChangeoverCostForAllDriver() {
        ...
        for (int d = 0; d < this.pathForDrivers.size() ; d++) {
            //2024.9.24 Correct the error should be < this.pathForDrivers.size() not .size()-1
            ArrayList<TripWithDriveInfos> pathForDriver = this.pathForDrivers.get(d).getDriverPath();
            for (int i=0;i<pathForDriver.size()-1;i++){
                ...
                if ((workingStatusInFormerTrip!=workingStatusInLatterTrip)||(idOfVehicleInFormerTrip!=idOfVehicleInLatterTrip)) {
                   // System.out.println("changOverHappen for driver_"+d+" with trip_"+idOfFormerTrip+" to trip_"+idOfLatterTrip);
                    changeoverCost = changeoverCost + this.instance.getCostForChangeOver(idOfFormerTrip, idOfLatterTrip);
                ...
                //we should remove the passenger change vehicle case;//2024.10.22
                ...
                // and add the driver using same vehicle but vehicle not consecutively perform the arc

                //2024.10.22
                //even the same status and same vehicle, but the vehicle doesnt perform successive then it should have change over cost according to the model constraints 27
    ...
    public int getNbDriversInSolution(){return this.pathForDrivers.size();}
    public int getNbVehiclesInSolution(){return this.pathForVehicles.size();}
    ...
    public boolean isSolutionFeasible(boolean verbose) {
        ...
        for (PathForDriver p : pathForDrivers) {//check all paths & find for this driver which is labeled as p
            for (int i = 0; i < p.getDriverPath().size()-1; i++) {
                if (p.getConnectionTimeInPathForDriver(i) <= this.instance.getShortConnectionTimeForDriver()) { //then find all the short connection time
                    ...
                    Trip lFirst = p.getTripFromDriverPath(i);
                    Trip lSecond = p.getTripFromDriverPath(i+1);
...
//        PathForVehicle pathForTheFirstAircraft = new PathForVehicle(instance, 0);
//        Trip trip1= instance.getTrip(6);
//        Trip trip2= instance.getTrip(5);
//        Trip trip3= instance.getTrip(4);
//        Trip trip4= instance.getTrip(3);
//
//        pathForTheFirstAircraft.addTripInVehiclePath(trip1);
//        pathForTheFirstAircraft.addTripInVehiclePath(trip2);
//        pathForTheFirstAircraft.addTripInVehiclePath(trip3);
//        pathForTheFirstAircraft.addTripInVehiclePath(trip4);
//
//        PathForVehicle pathForTheSecondAircraft = new PathForVehicle(instance, 1);
//        Trip trip5= instance.getTrip(0);
//        Trip trip6= instance.getTrip(1);
//        Trip trip7= instance.getTrip(2);
//
//
//        pathForTheSecondAircraft.addTripInVehiclePath(trip5);
//        pathForTheSecondAircraft.addTripInVehiclePath(trip6);
//        pathForTheSecondAircraft.addTripInVehiclePath(trip7);
//
//
//
//        solution.pathForVehicles.add(pathForTheFirstAircraft);
//
//
//        solution.pathForVehicles.add(pathForTheSecondAircraft);
//
//        PathForDriver pathForTheFistCrew = new PathForDriver(instance, 0);
//        pathForTheFistCrew.addTripInDriverPath(instance.getTrip(6));
//        pathForTheFistCrew.addTripInDriverPath(instance.getTrip(1));
//        pathForTheFistCrew.addTripInDriverPath(instance.getTrip(2));
//
//        solution.pathForDrivers.add(pathForTheFistCrew);
//
//        PathForDriver pathForTheSecondCrew = new PathForDriver(instance, 1);
//        pathForTheSecondCrew.addTripInDriverPath(instance.getTrip(0));
//        pathForTheSecondCrew.addTripInDriverPath(instance.getTrip(5));
//        pathForTheSecondCrew.addTripInDriverPath(instance.getTrip(4));
//        pathForTheSecondCrew.addTripInDriverPath(instance.getTrip(3));
//
//        solution.pathForDrivers.add(pathForTheSecondCrew);
//
