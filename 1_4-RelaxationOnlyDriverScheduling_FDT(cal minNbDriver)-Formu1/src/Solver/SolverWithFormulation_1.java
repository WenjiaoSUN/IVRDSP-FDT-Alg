...
//                    System.out.println("varLeading(NonLeading)" + varLeading[Integer.valueOf(values[m])][Integer.valueOf(values[m + 1])]);
                ...
                while (line.startsWith("//") || line.startsWith("The") ||line.startsWith("vehicle")|| line.isEmpty()) {
                    line = bufferedReader.readLine();
                }
...
//        for (int v = 0; v < nbVehicle; v++) {
//            for (int i = 0; i < nbTrip; i++) {
//                int duration_i = instance.getDistance(instance.getTrip(i).getIdOfStartCity(), instance.getTrip(i).getIdOfEndCity());
...
//        InstanceReader reader = new InstanceReader("largerExample.txt");
        InstanceReader reader1 = new InstanceReader("inst_nbCity03_Size90_Day1_nbTrips080_combPer0.0_TW6.txt");

        Instance instance = reader1.readFile(); //这个语句将文本的内容就读出来了
        System.out.println(instance);

        String warmStartFileName = "feaSol_nbCity03_Size90_Day1_nbTrips080_combPer0.0_TW6.txt";


        SolverWithFormulation_1 solverWithFormulation1 = new SolverWithFormulation_1(instance, warmStartFileName , false);

        // print solution
        Solution sol = solverWithFormulation1.solveWithCplex();
        if (sol != null) {
            sol.printInfile("sol_nbCity05_Size180_Day1_nbTrips040_combPer0.25_TW0.txt");
        }
