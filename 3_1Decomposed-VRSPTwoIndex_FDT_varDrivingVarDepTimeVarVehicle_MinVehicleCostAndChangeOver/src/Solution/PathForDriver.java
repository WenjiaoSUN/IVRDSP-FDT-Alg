...
public class PathForDriver {
    ...
    public PathForDriver(Instance instance, int idOfDriver) {
        ...
        this.driverPathWithInfos = new ArrayList<TripWithDriveInfos>();
...
//    }
    ...
    public int getIdDepot(){
        int idDepot=Integer.MAX_VALUE;
        int idStartCityFirstTrip=this.driverPathWithInfos.get(0).getIdOfStartCity();
        for(int k=0;k<instance.getNbDepots();k++){
            Depot depot=instance.getDepot(k);
            int idCityAsDepot=depot.getIdOfCityAsDepot();
            if(idCityAsDepot==idStartCityFirstTrip){
                idDepot=depot.getIdOfDepot();
            }
        }
        return  idDepot;
    }
    ...
    public String toString() {
        String s="driver "+idOfDriver+ " "+ driverPathWithInfos.size()+" "+this.getIdDepot();
        for(int i = 0; i< driverPathWithInfos.size(); i++){
            s+=" "+ driverPathWithInfos.get(i).getIdOfTrip()+" "+
                    driverPathWithInfos.get(i).getIdOfVehicle()+" "+
                    driverPathWithInfos.get(i).getDrivingStatus()+" "
                    +driverPathWithInfos.get(i).getStartingTime();
        }
