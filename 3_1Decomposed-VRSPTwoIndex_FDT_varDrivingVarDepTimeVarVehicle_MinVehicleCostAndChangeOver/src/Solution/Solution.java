...
public class Solution {
        ...
        for (int d = 0; d < this.pathForDrivers.size(); d++) {
            ...
            for (int i = 0; i < pathForDriver.size() - 1; i++) {
                ...
                if ((idOfVehicleInFormerTrip != idOfVehicleInLatterTrip) || (workingStatusInFormerTrip != workingStatusInLatterTrip)) {
                    ...
                    if((idOfVehicleInFormerTrip != idOfVehicleInLatterTrip) && (workingStatusInFormerTrip != workingStatusInLatterTrip)){
                        System.out.println("Until now total changOver Cost is " + changeoverCost + " because of " + "idOfDriver_" + d + " formerTrip_" + idOfFormerTrip + " latterTrip_" + idOfLatterTrip+" change status and vehicle in the same time");
                    }else {
                        if ((idOfVehicleInFormerTrip != idOfVehicleInLatterTrip)) {
                            System.out.println("Until now total changOver Cost is " + changeoverCost + " because of " + "idOfDriver_" + d + " formerTrip_" + idOfFormerTrip + " latterTrip_" + idOfLatterTrip+" change vehicle");
                        }

                        if ((workingStatusInFormerTrip != workingStatusInLatterTrip)) {
                            System.out.println("Until now total changOver Cost is " + changeoverCost + " because of " + "idOfDriver_" + d + " formerTrip_" + idOfFormerTrip + " latterTrip_" + idOfLatterTrip+" change working status");
                        }
                    }
                }
                if (workingStatusInFormerTrip == false && workingStatusInLatterTrip == false && (idOfVehicleInFormerTrip != idOfVehicleInLatterTrip)) {
                    changeoverCost = changeoverCost - this.instance.getCostForChangeOver();
                    System.out.println("Until now total changOver Cost is " + changeoverCost + " because of remove penalty for driver" + "idOfDriver_" + d + " formerTrip_" + idOfFormerTrip + " latterTrip_" + idOfLatterTrip+" because the cost is caused on passenger change vehicle");
                }
                ...
                if (workingStatusInFormerTrip == true && workingStatusInLatterTrip == true && (idOfVehicleInFormerTrip == idOfVehicleInLatterTrip)) {
                    ...
                    if (whetherNeedAddPenalty) {
                        changeoverCost = changeoverCost + this.instance.getCostForChangeOver();
                        System.out.println("Until now total changOver Cost is " + changeoverCost + " because of add penalty for driver" + "idOfDriver_" + d + " formerTrip_" + idOfFormerTrip + " latterTrip_" + idOfLatterTrip+" even drive by same vehicle , but they are not continuous in vehicle route");
                    }
                }
            }
        }
    ...
    public void printInfile(String fileName) {
        ...
        try {
            ...
            writer.println("//The changeover cost for all drivers: "+ this.getTotalChangeoverCostForAllDriver());
