...
import Solver.SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost;
...
//                String solutionFileName = "solution_" + fileName;
                String solutionFileName = "FirstSolutionGivenByTwoIndex_" + fileName;//
...
////                        //the following two line is when we solve with no warm file and also no consider changeoverCost
                        SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost solverBasedOnDriverSchedule = new SolverTwoIndexBasedOnDriverScheduleMinVehicleIdleTimeCostAndChangeOverCost(instance, driverSchedules, whetherSolveRelaxationProblem);
...
//                    writer.print(solver.getGap() + ",");
                        ...
                        if (whetherSolveRelaxationProblem == true) {
                            ...
                            if (sol != null) {
                                ...
                                writerSolution.println("//vehicle routes in the solution: idVehicle, nbTrips, idStDepot, [idTrips depTime], idEnDepot");
                                for(int r=0;r<sol.getNbVehiclesInSolution();r++){
                                    writerSolution.println(sol.getPathForVehicles().get(r));
                                }
                                writerSolution.println("//driver schedules in the solution: idDriver, nbTrips, idStDepot, [idTrips idVehicle status depTime]");
                                for (int s=0; s<sol.getNbDriversInSolution();s++){
                                    writerSolution.println(sol.getPathForDrivers().get(s));
                                }
