...
public class DriverSchedule {
    ...
    public boolean whetherIsOriginOfAShortConnection(TripWithWorkingStatusAndDepartureTime combinedTripWithWorkingStatusAndDepartureTime){
        boolean whetherIsOriginOfAShortConnection=false;
        int depTimeCurrent=combinedTripWithWorkingStatusAndDepartureTime.getDepartureTime();
        int durationCurrent=combinedTripWithWorkingStatusAndDepartureTime.getDuration();
        int indexOfTrip=Integer.MAX_VALUE;
        for(int i=0;i<tripWithWorkingStatusAndDepartureTimeArrayList.size();i++){
            TripWithWorkingStatusAndDepartureTime tripWithWorkingStatusAndDepartureTime=this.tripWithWorkingStatusAndDepartureTimeArrayList.get(i);
            if(tripWithWorkingStatusAndDepartureTime.getIdOfTrip()==combinedTripWithWorkingStatusAndDepartureTime.getIdOfTrip()){
                indexOfTrip=i;
            }
        }
        if(indexOfTrip<this.tripWithWorkingStatusAndDepartureTimeArrayList.size()-1){
            TripWithWorkingStatusAndDepartureTime latterTrip=this.tripWithWorkingStatusAndDepartureTimeArrayList.get(indexOfTrip+1);
            int depTimeLatter=latterTrip.getDepartureTime();
            if((depTimeLatter-depTimeCurrent-durationCurrent)<=instance.getShortConnectionTimeForDriver()){
                whetherIsOriginOfAShortConnection=true;
            }
        }
        return whetherIsOriginOfAShortConnection;
    }

    public boolean whetherADestinationOfAShortConnection(TripWithWorkingStatusAndDepartureTime combinedTripWithWorkingStatusAndDepartureTime){
        boolean whetherADestinationOfAShortConnection=false;
        int depTimeCurrent=combinedTripWithWorkingStatusAndDepartureTime.getDepartureTime();
        int durationCurrent=combinedTripWithWorkingStatusAndDepartureTime.getDuration();
        int indexOfTrip=Integer.MAX_VALUE;
        for(int i=0;i<tripWithWorkingStatusAndDepartureTimeArrayList.size();i++){
        ...
        if(indexOfTrip>0){
            TripWithWorkingStatusAndDepartureTime formerTrip=this.tripWithWorkingStatusAndDepartureTimeArrayList.get(indexOfTrip-1);
            int depTimeFormer=formerTrip.getDepartureTime();
            int durationOfFormer=formerTrip.getDuration();
            if((depTimeCurrent-depTimeFormer-durationOfFormer)<=instance.getShortConnectionTimeForDriver()){
                whetherADestinationOfAShortConnection=true;
            }
        }
        return whetherADestinationOfAShortConnection;
    }
