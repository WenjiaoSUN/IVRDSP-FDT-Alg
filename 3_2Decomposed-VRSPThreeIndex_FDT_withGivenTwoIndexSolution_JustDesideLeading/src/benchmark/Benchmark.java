...
public class Benchmark {
    ...
    private static String twoIndexSolDirectory = "twoIndex"; //in order to read the schedules obtained from the first phase
...
//    private static String statisticsFile="statistics.csv";// here is to give a statistics result on the solution
    ...
    public static void read(String[] args) {
        ...
        if (options.containsKey("-twoIndex")) {
            twoIndexSolDirectory = options.get("-twoIndex");
        }
    ...
    public static void solveFromFiles() throws IloException, IOException {
        ...
        if (f.isDirectory()) {
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                ...
                String twoIndexSolutionFileName = "FirstSolutionGivenByTwoIndex_" + fileName;// this is for read the schedules obtained from CG
                String twoIndexSolutionFilePath = twoIndexSolDirectory + "\\" + twoIndexSolutionFileName;
                System.out.println(twoIndexSolutionFilePath);
                System.out.println(twoIndexSolutionFileName);
...
//                String solutionFileName = "solution_" + fileName;
                ...
                try {
                    ...
                    if (fileName.startsWith("inst_") && fileName.endsWith(".txt")) {
                        ...
                        InputInformsReader inputInformsReader = new InputInformsReader(twoIndexSolutionFilePath, instance);
...
//                    writer.print(solver.getGap() + ",");
                        ...
                        if (whetherSolveRelaxationProblem == true) {
                            ...
                            if (sol != null) {
                                ...
                                writerSolution.println("cost "+sol.getTotalCost());
                                writerSolution.println("//vehicle routes in the solution: idVehicle, nbTrips, [idTrips departureTime]");
                                for(int r=0;r<sol.getNbVehiclesInSolution();r++){
                                    writerSolution.println("vehicle "+sol.getPathForVehicles().get(r));
                                }
                                writerSolution.println("//driver schedules in the solution: idDriver, nbTrips, [idTrip idVehicle drivingStatus departureTime]");
