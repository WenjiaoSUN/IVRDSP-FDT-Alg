...
public class Solution {
    ...
    public ArrayList<TripWithStartingInfos> getAllTripWithStartingTimeInfos(){
        ArrayList<TripWithStartingInfos> allTripWithDepartureTimeInfosArrayList= new ArrayList<>();
        for(int i=0; i<instance.getNbTrips();i++) {
            int idTrip=instance.getTrip(i).getIdOfTrip();
            for (int v = 0; v < this.getNbVehiclesInSolution(); v++) {
                PathForVehicle pathForVehicle = this.pathForVehicles.get(v);
                for (int t = 0; t < pathForVehicle.getTripsInPath().size(); t++) {
                    TripWithStartingInfos tripWithStartingInfos = pathForVehicle.getTripWithStartingInfos(t);
                    if(tripWithStartingInfos.getIdOfTrip()==idTrip) {
                        allTripWithDepartureTimeInfosArrayList.add(tripWithStartingInfos);
                    }
                }
            }
        }
        return allTripWithDepartureTimeInfosArrayList;
    }
